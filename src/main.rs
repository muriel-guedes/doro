...
use ra_ap_ide::{FilePosition, TextSize};
use ra_ap_project_model::{
    CargoConfig, CargoWorkspace, CfgOverrides, ManifestPath, ProjectWorkspace, Sysroot,
...
fn main() {
    ...
    let cargo_toml = ManifestPath::try_from(AbsPathBuf::assert(
    ...
    let (analysis_host, vfs, _proc_macro_server) = load_cargo::load_workspace(ws, &extra_env, &load_config).unwrap();
